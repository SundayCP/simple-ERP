<template>
  <div class="details-category">
    <h1 class="page-title">聚乐页面配置/页面配置/详情配置</h1>
    <div class="container">
      <el-form :inline="true">
      ...
      <template>
        <el-table
          ...
          min-width="110"
            label="排序">
            <template slot-scope="scope">
              <span v-show="(scope.$index!==tableData.length-1)||(currentPage!==Math.ceil(pageTotal/pageSize))"
                    class="operation-btn" style="color: #1890ff"
                    @click="getDetailsMoveDown(scope.row)">下移</span><span
              v-show="((scope.$index!==tableData.length-1)||(currentPage!==Math.ceil(pageTotal/pageSize)))&&((scope.$index!==0)||(currentPage!==1))">&nbsp;&nbsp;|&nbsp;&nbsp;</span><span
              v-show="(scope.$index!==0)||(currentPage!==1)" style="color:#1890ff;"
              @click="getDetailsMoveUp(scope.row)"
    ...
    <el-dialog :title="dialogTitle" width="422px"
      ...
      <el-form :model="form">
        <el-form-item label="业务名称：">
          <el-input style="width: 317px" v-model="form.name" placeholder="">
          </el-input>
        </el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        options: [],
        currentPage: 1,
        uploadData: {name: '', linkUrl: '', id: '',pid:'',itemCategoryId:''},
        fileList: [],
    ...
    this.uploadData.pid=this.$route.query.businessId
      let _this = this;
      // this.getDetailsDifferServ()
      this.selectObj = {
        businessId: _this.$route.query.businessId,
        currentPage: 1,
        pageSize: this.pageSize,
      }
      this.getDetailsCategoryList(this.selectObj)
    },
    methods: {
      ...
      // getDetailsDifferServ() {
      //   core.detailsDifferServ().then(data => {
      //     if (data.code && '00' === data.code) {
      //     } else {
      //       this.$message.error(data.message)
      //     }
      //   })
      // },
      getDetailsMoveUp(val) {
        core.detailsMoveUp({id: val.id}).then(data => {
          if (data.code && '00' === data.code) {
      ...
      getDetailsMoveDown(val) {
        core.detailsMoveDown({id: val.id}).then(data => {
          if (data.code && '00' === data.code) {
            this.getDetailsCategoryList(this.selectObj)
      ...
      getDetailsCategoryList(opts) {
      this.loading=true
        core.detailsCategoryList(opts).then(data => {
          if (data.code && '00' === data.code) {
            this.loading=false
      ...
      submitUpload() {
        ...
        if (this.dialogTitle === '编辑信息') {
          if (this.fileList.length === 0) {
            core.detailsEditInformation({
              name: this.form.name,
              linkUrl: this.form.linkUrl,
              value: this.form.value,
              id: this.form.id,
              pid:this.$route.query.businessId
            }).then(data => {
              if (data.code && '00' === data.code) {
...
<style>
  .details-category .el-dialog__body {
    padding: 0px 16px;
  }
</style>
